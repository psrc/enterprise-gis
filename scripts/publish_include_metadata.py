...
#gis = GIS(portal_url, username='SCoe@PSRC.org', password='')

####### Testing 
fail_list = []
for layer in map.listLayers():
    map.removeLayer(layer)
dataset = 'ElmerGeo.DBO.political'    
dataset_name = 'branch_version_test'
centers = r"ElmerGeo.DBO.taz_branch_test"
#outdir = r'C:\Users\scoe\Documents\publish_elmer_geo\service_definitions'
...
#desc = arcpy.Describe(elmer_geo_conn_path)
#test = arcpy.MakeFeatureLayer_management(r"C:\Stefan\publish_elmer_geo\elmer_geo\elmer_geo_conn.sde\ElmerGeo.DBO.tract20centroids")
fail_list = []
for dataset in arcpy.ListDatasets():
    dataset_name = dataset.split(".")[2]
    for fc in arcpy.ListFeatureClasses("*", "", dataset_name):
        if fc not in skip_layers and dataset_name != 'test':
            for layer in map.listLayers():
                map.removeLayer(layer)
            test = Path(elmer_geo_conn_path)/dataset/fc
            lyr = map.addDataFromPath(test)
            aprx.save()
            layer_name = fc.split(".")[2]
            tags = f"ElmerGeo, {dataset_name}"
            try:
                publish_to_portal(map, outdir, layer_name, tags, dataset_name.capitalize())
            except:
                fail_list.append(layer_name)
...
# new_md.description = 'This is a description'



# tgt_item_md = md.Metadata(test)
# if not tgt_item_md.isReadOnly:
#     tgt_item_md.copy(new_md)
#     tgt_item_md.save()
#     # sync must go after updating metadata:
...
# now share this layer with gropu:
# layer_items = gis.content.search("test_feature_service", item_type='Feature Service', max_items=100)
# service_id = layer_items[0].service_id
# group_manager = gis.groups
# group_id = group_manager.search(query='ElmerGeo Admin')[0]
